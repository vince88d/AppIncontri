...
export default function GroupPrivateChatScreen() {
  ...
  const threadId = useMemo(() => (params.id ? String(params.id) : null), [params.id]);
  const groupId = useMemo(() => (params.groupId ? String(params.groupId) : null), [params.groupId]);
  const otherId = useMemo(() => (params.otherId ? String(params.otherId) : null), [params.otherId]);
  const otherName = params.otherName ? String(params.otherName) : 'Utente';
  const groupTitle = params.groupTitle ? String(params.groupTitle) : 'Gruppo';
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, 'groupRooms', groupId, 'privateThreads', threadId, 'messages'),
  ...
  const handleSend = async () => {
    if (!groupId || !threadId || !user?.uid || !otherId) return;
    const trimmed = input.trim();
    if (!trimmed) return;
    setSending(true);
    setInput('');
    const myName = profile?.name || user.displayName || 'Tu';

    try {
      await setDoc(
        doc(db, 'groupRooms', groupId, 'privateThreads', threadId),
        {
          groupId,
          groupTitle,
      ...
      await addDoc(collection(db, 'groupRooms', groupId, 'privateThreads', threadId, 'messages'), {
        text: trimmed,
        senderId: user.uid,
        senderName: myName,
      ...
      <View style={[styles.messageRow, isMine ? styles.messageRowMine : styles.messageRowOther]}>
        ...
        >
          <View style={styles.bubbleHeader}>
            <Text style={[styles.sender, { color: isMine ? '#fff' : palette.text }]}>
              {senderLabel}
            </Text>
          ...
          ListEmptyComponent={
            loading ? null : (
              <View style={styles.empty}>
                <Ionicons name="mail-outline" size={40} color={palette.muted} />
